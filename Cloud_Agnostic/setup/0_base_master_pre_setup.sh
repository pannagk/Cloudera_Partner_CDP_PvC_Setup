...
ansible_config() {
    ...
    WORKERS_VARS_LINE="[workers:vars]\n$HOST_KEY_CHECK\n$ANSIBLE_USER\n$ANSIBLE_PASSWORD\n$ANSIBLE_SUDO\n"
    echo -e $WORKERS_LINE >> /tmp/CDP_PvC/ansible/hosts
    echo -e $WORKERS_VARS_LINE >> /tmp/CDP_PvC/ansible/hosts

    ALL_LINE="[cluster]\n${MASTER_IP}\n${CDP_BASE_WORKER1}\n${CDP_BASE_WORKER2}\n${CDP_BASE_WORKER3}\n"
    echo -e $ALL_LINE >> /tmp/CDP_PvC/ansible/hosts

    IPA_LINE="\n[ipaserver]\n${FREEIPA_SERVER}"
    IPA_VARS_LINE="[ipaserver:vars]\n$HOST_KEY_CHECK\n$ANSIBLE_USER\n$ANSIBLE_PASSWORD\n$ANSIBLE_SUDO\n"
    echo -e $IPA_LINE >> /tmp/CDP_PvC/ansible/hosts
    echo -e $IPA_VARS_LINE >> /tmp/CDP_PvC/ansible/hosts
}
